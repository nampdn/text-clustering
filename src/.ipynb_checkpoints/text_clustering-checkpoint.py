...
class ClusterClassifier:
    ...
    def summarize(self, texts, labels):
        ...
        for label in range(unique_labels):
            ...
            response = 'unknown-label' # client.text_generation(request)
            if label == 0:
                print(f"Request:\n{request}")
            cluster_summaries[label] = response# self._postprocess_response(response)
    ...
    def show(self, interactive=False):
        df = pd.DataFrame(
            data={
                ...
                "content_display": [
                    textwrap.fill(txt, 64) for txt in self.texts
                ],
            }
        )
    ...
    def _show_mpl(self, df):
        ...
        for label in self.cluster_summaries.keys():
            if label == -1:
                continue
            summary = str(label) #self.cluster_summaries[label]
